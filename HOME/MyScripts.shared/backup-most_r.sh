...
# Immer kein delete, weil sonst beim umstecken der disk an anderes Geraet
# unbeabsichtigt einiges geloscht wird,
# da der Inhalt der betreffenden Verzeichnisse (bes. 18*) von Geraet zu Geraet unterschiedlich ist.
# Wenn Loeschen im Quellverzeichnis tatsaechlich aufs Ziel uebertragen werden soll,
# dann manuelles rsync ausfuehren.
# #delete=--delete
delete=""
errr=$errdir/err.rsync-ohne-backup
rsyncdir_ohne_backup=$disk/Rsync-ohne-backup

echo "$bold $fg[green] \n\n STATUS ************** rsync nach $rsyncdir_ohne_backup ************** \n\n $reset  " | tee -a $errr | tee -a $err

date | tee -a $errr
set_start_time

echo "$fg["yellow] | tee -a $errr
echo \
rsync $rsync_options_basic $delete --exclude-from=$tmp1 $rsync_ohne_backup $rsyncdir_ohne_backup \
        | tee -a $errr

echo "$fg[magenta]" | tee -a $errr

rsync $rsync_options_basic $delete --exclude-from=$tmp1 $rsync_ohne_backup $rsyncdir_ohne_backup \
        |& grep -v -e "symlink" -e "cannot delete non-empty directory" -e ".*\/$" 2>&1 | tee -a $errr

echo " $reset " | tee -a $errr
print_time_used $rsyncdir_ohne_backup  | tee -a  $errr | tee -a $err

#----------------------------------------------------
errb=""
if [[ "$ziel" == "$defaultziel" ]]; then
        errb=$errdir/err.rsync-backupdir-u.a.-auf-zweite-disk

        zieltmp=$backup_von_backup
        echo "$bold $fg[cyan] \n\n STATUS ************** rsync Einiges von $ziel nach $zieltmp \n\n $reset  " | tee -a $errb | tee -a $err

        for indir in `basename $backup_1stbase` `basename $rsyncdir_ohne_backup` 09_Alles_andere_in_Cloud 17_Nicht-in-Cloud-OHNE-FLAC 16_Sonja ; do

                echo "$bold $fg[green] \n\n           STATUS ************** rsync $indir von $ziel nach $zieltmp \n\n $reset  " | tee -a $errb | tee -a $err
                date | tee -a $errb
                set_start_time
                echo "$fg[yellow] " | tee -a $errb
                echo \
                rsync $rsync_options_basic --delete  $ziel/$indir $zieltmp \
                        | tee -a $errb
                echo "$fg[$standard_fg_color] " | tee -a $errb

                rsync $rsync_options_basic --delete  $ziel/$indir $zieltmp \
                        |& grep -v -e "symlink" -e "cannot delete non-empty directory" -e ".*\/$" 2>&1 | tee -a $errb

                print_time_used $indir | tee -a  $errb | tee -a $err
                echo -e "\n"
        done

else
        echo -e "\n\n $bold $fg[yellow] !!!!!!!!!!!! Nuetzlich waere manuell folgendes auszufuehren: " | tee -a $err
        echo -e "rsync -uav  $defaultziel/`basename $backup_1stbase` $ziel # AUF JEDEN FALL OHNE --delete !!!! \n" | tee -a $err
        echo -e "                          ... sonst werden die Sicherungen von backup-most.sh --enforce_backup toi geloescht \n" | tee -a $err
        echo "ggf nachtraeglich Foto-Konserven manuell entfernen mit" | tee -a $err
        echo "find $backup_1stbase -name Fotos-Konserven -exec rm -rf '{}' \\;" | tee -a $err
        echo "oder vorher schon ausschliessen mit:" | tee -a $err
        echo -e "rsync --exclude=Fotos-Konserven -uav  $defaultziel/`basename $backup_1stbase` $ziel  \n\n$reset " | tee -a $err
fi

#----------------------------------------------------

echo -e "$bold $fg[magenta] \n\n **************  Errors (if any)  >>>>>>>>>>>>> \n"
grep -i error $err $errc $errr $errb  |grep -v -e grep -e "if any" -e "Error file" -e "some files/attrs" $irrelevant_err_warn
echo grep -i error $err $errc $errr $errb
echo "\n<<<<<<<<<<<<< $reset $fg[$standard_fg_color] \n\n"
for e in  $err $errc $errr $errb ; do
        grep -v "\/$" $e > $e.ohne-dirs
        echo more $e.ohne-dirs
done
echo grep deleting $err $errc $errr $errb

echo -e "\n----------------------\n"

#----------------------------------------------------

anz_backup=`find $backupdir -type f -print |wc |gawk '{print $1}'`
anz_del=`grep deleting $err $errc $errr $errb |wc |gawk '{print $1}'`


msg1=" $task on $DEVICE with target $ziel "
m="$dry_run$no_lists$enforce_backup"
if [[ "$m" != "" ]]; then
        msg2=" with Options  $dry_run  $no_lists  $enforce_backup "
else
        msg2=""
fi
echo -e "\n------------------------------------------\n\n" |tee -a $disk/BACKUP.HISTORY.txt  | tee -a $err | tee -a $HOME_BACKUP_HISTORY
print_time_used "$msg1" "$msg2" $initial_timestamp  |tee -a $disk/BACKUP.HISTORY.txt  | tee -a $err | tee -a $HOME_BACKUP_HISTORY

echo "\t\t Protocols and errors are in " `dirname $err` |tee -a $disk/BACKUP.HISTORY.txt  | tee -a $HOME_BACKUP_HISTORY
echo "\t\t A total of $anz_del Files were deleted " |tee -a $disk/BACKUP.HISTORY.txt  | tee -a $err | tee -a $HOME_BACKUP_HISTORY
for e in $err $errc $errr $errb ; do
        anz_del=`grep deleting $e |wc |gawk '{print $1}'`
        echo "\t\t\t$e : $anz_del deletions"  | tee -a $disk/BACKUP.HISTORY.txt | tee -a $err | tee -a $HOME_BACKUP_HISTORY
done

echo "\t\t A total of $anz_backup Files moved into backupdir " |tee -a $disk/BACKUP.HISTORY.txt | tee -a $err | tee -a $HOME_BACKUP_HISTORY
cd $backupdir
typeset -a D=( `find . -maxdepth 1 -type d -print`)
for d in $D ; do
        if [[ "$d" != "." ]]; then
                anz_backup=`find $d -type f -print |wc |gawk '{print $1}'`
                echo -e "\t\t\t$d :  $anz_backup  Files moved to backupdir  $backupdir/$d" | tee -a $err  | tee -a   $disk/BACKUP.HISTORY.txt | tee -a $HOME_BACKUP_HISTORY
        fi
done


echo -e "\n$bold$fg[magenta]Rsync-ohne-backup immer ohne --delete. Ggf rsync --delete manuell starten.$reset "
